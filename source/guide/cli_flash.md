...
离线命令（`probe` / `dry-run`）输出计划文本后退出；在线命令在目标上执行。在线命令执行时在终端逐阶段显示进度，结束后输出一行执行汇总（各阶段实际执行的字节数与速度）；输出重定向到文件或管道时不做单行刷新，仅保留各阶段的定格行，便于查看日志。
...
默认开启跳过优化：擦除前将待烧录扇区与镜像预期内容比对，内容已一致的扇区跳过擦除、编程与校验，空白扇区及内容仍为擦除值的页跳过擦除与编程，重复下载明显加快；`--force-prog` 可禁用。另外，待烧录范围覆盖整个 bank 且算法支持整片擦除时，会自动用一次整片擦除替代逐扇区擦除。
